...
void sgemv(const float *A, const float *x, float *y, bool transA, int M, int N,
           bool is_bias, const float *bias, bool is_relu) {
  if (transA) {
    LOG(FATAL) << "ERROR: sgemv, transA is not supported now\n";
  }
  if (is_bias) {
    //! with bias
    if (is_relu) {
...
#define SGEMV_KERNEL_8                                                         \
  ...
  "bne 1b                     \n" /* jump to main loop pair add to final res*/ \
                                                                               \
  "2:                         \n"  /* reduce to scale */                       \
...
#define SGEMV_KERNEL_1                                                         \
  ...
  "bne 1b                     \n" /* jump to main loop, pairAdToFinalRes */    \
                                                                               \
  "2:                         \n" /* reduce to scale */                        \
